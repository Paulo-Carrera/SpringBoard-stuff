describe('Meme Generator', ()=>{
    ...
    it('adds a meme when the form is submitted', ()=>{
        cy.get('.meme').should('not.exist');
        addMeme();
        cy.get('.meme').should('exist');
    });

    it('removes a meme when the meme is clicked', ()=>{
        addMeme();
        cy.get('.meme').click();
        cy.get('.meme').should('not.exist');
    });
    ...
    function addMeme() {
        cy.get('#image').type('https://www.meme-arsenal.com/memes/6d415311d80dbda72c104f875d3ff23c.jpg')
