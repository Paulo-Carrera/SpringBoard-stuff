...
// before each test, create test users and grant them tokens.
...
describe('GET /topsecret', function(){
    test('returns made it!', async function(){
        const response = await request(app).get('/topsecret').send({
            _token : testUserToken });
        expect(response.statusCode).toBe(200);
    });

    test('returns 401 when logged out', async function(){
        const response = await request(app).get('/topsecret'); // no token being sent
...
afterEach(async function(){
    // delete any data created by these tests
